...
function AdminRoom() {
  ...
  const visibleProducts = roomss.slice(startIndex, startIndex + itemsPerPage);
      ...
      <div className="container-adminRoom">
        ...
        <Table className="table">
          ...
          <tbody>
            {rooms?.map((room) => (
              <tr key={room.room_id}>
                <td><img src={room.img_rooms} width={100} height={100}/></td>
                <td>{room.direction}</td>
                <td>{room.capacity}</td>
                <td>{room.description}</td>
                <td>{room.floor_area}m2</td>
                <td>{room.price_room.toLocaleString()}VNĐ</td>
                <td>{room.status_room === 0 ? <p style={{fontWeight : 500}}>Phòng trống</p> : <p style={{fontWeight : 500}}>Đã đặt</p>}</td>
