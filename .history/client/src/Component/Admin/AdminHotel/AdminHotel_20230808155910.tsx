...
function AdminHotel() {
  ...
  const showEditModal = (id: number) => {
    const hotelToEdit = searchResults.find(
      (hotel: Hotel) => hotel.hotel_Id === id
    );
    if (hotelToEdit) {
  ...
  const handleEditOk = () => {
    try {
     const {imgRoom , nameRoom, availability , location , price} = editHotelData;
      if(!imgRoom || !nameRoom || !availability || !location || !p)
