...
const Bookroom = () => {
  ...
  const itemsPerPage = 5; // Số lượng khách sạn hiển thị trên mỗi trang
  ...
  const loadHotel = async () => {
    try {
      const response = await axios.get(`http://localhost:4000/hotels/location?location=${searchResult?.location}`);
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`http://localhost:4000/hotels/location/search?search=${searchValue}`);
