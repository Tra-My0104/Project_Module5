...
const Detailroom = () => {
  ...
  const loadRate = async () => {
    try {
      let response = await axios.get<any>(`http://localhost:4000/rateting/${detailRoom?.hotel_id}`)
      if(response.data){
      ...
      <div className="main">
        ...
        {rooms?.map((room: any) => (
          <div className="containerItemRoom">
            <div style={{display : "flex"}}>
             <span>Phòng số</span> <p className="name_room">{room.name_room}</p>
              <div>
              <img
                src={room.img_rooms}
                style={{
        ...
        <div className="containerFeedback">
          ...
          <div className="feedback">
            ...
            <button className="deleteComment" onClick={() => handleDeleteComment(rates.rateting_id)}><i className="fa-solid fa-trash" style={{color: "#ff0000" , fontSize : "larger"}}></i></button>
            </div>
          </div>
          ))}
        </div>
