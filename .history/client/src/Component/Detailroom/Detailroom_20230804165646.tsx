...
const Detailroom = () => {
  ...
  console.log("detailRoom?.hotel_id:", detailRoom?.hotel_id);
  console.log("userInfor.UserId:", userInfor.UserId);
  const handleRate = async () => {
     try {
      const response = await axios.post(`http://localhost:4000/rateting/` , rateting)
    ...
    <>
      ...
      <div className="main">
        ...
        <div className="rating-form">
          <h4 style={{marginLeft : "74px" , fontWeight: "400"}}>Đánh giá khách sạn</h4>
          <Rate className="rate" value={rate} onChange={(value) => setRate(value)}/><br />
          <TextArea rows={4} value={comment} onChange={(e) => setComment(e.target.value)}/> <br />
          <button className="btn_feed_back" onClick={() => detailRoom?.hotel_id && userInfor?.UserId && handleRate(detailRoom.hotel_id)}>
  Gửi đánh giá
</button>
        </div>
