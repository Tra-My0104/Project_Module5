...
const Adminuser = () => {
  ...
  if(tokenJson){
   tokenString = JSON.parse(tokenJson)
  };
  ...
  const handleSave = async () => {
    try {
      await axios.put(`http://localhost:4000/users/edit_user/${editUser.UserId}`, editUser , {headers: {Authorization: `Bearer ${tokenString}`}});
            ...
            {adminUser?.map((user: any, index: number) => (
              <tr key={index}>
                ...
                <td>
                  ...
                  <Modal show={show} onHide={handleClose} className="editUserAdmin">
