...
const Bookinginformation = () => {
  ...
  const loadRoom = async () => {
    const response = await axios.get(`http://localhost:4000/rooms/${id}`);
    console.log(response.data.findRoom)
    setBookingInfor(response.data.findRoom);
  };
  ...
  const handlePay = async() => {
     try {
      //  await axios.post("http://localhost:4000/booking-room" , bookingData)
      await axios.patch(`http://localhost:4000/rooms/status_room_2/${bookingInfor?.rooms_id}`)
    ...
    <>
      ...
      <div className="container-infor">
        ...
        <div className="inforHotel">
          ...
          <hr />
          <p>Số phòng : <span>{bookingInfor?.name_room}</span></p>
          <p>Ngày nhận phòng : <span>{searchInfor.dateForm}</span></p>
          <p>Số đêm: <span>{searchInfor.numberNight} đêm</span></p>
          <p>Ngày trả phòng: <span>{searchInfor.checkoutDate} đêm</span></p>
